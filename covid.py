...
def getNumCasesFromLine(oneLine):
    values = oneLine.split(",")
    a = values[2]
    return int(a)
...
def calcXCoord(oneLine):
    long = getLongFromLine(oneLine)
    x = (long + 150) * 500/100
    return int(x)
...
def calcYCoord(oneLine):
    lat = getLatFromLine(oneLine)
    y = 500 - (lat * 500/75)
    return int(y)
...
def drawMapForDate(searchDate):
    ...
    f = open("us.csv", "r")
    for x in f:
        oneDate = getDateFromLine(x)
        if oneDate == searchDate:
            drawInfection(x)
